...
export const CategoriesSlider = () => {
  const { categories, products, activeCategory, setActiveCategory, addToast } = useStore();
  ...
  const handleCategoryClick = (cat) => {
    ...
    addToast('Category Selected', `Showing ${cat.name}`);
  };
  ...
  return (
    <section className="py-6 px-4 sm:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-brand-green flex items-center justify-center transition-colors shadow-xs focus:outline-none cursor-pointer"
          ...
          <button
            ...
            className="w-8 h-8 rounded-lg bg-[#0b4d3c] text-white hover:bg-emerald-800 flex items-center justify-center transition-colors shadow-xs focus:outline-none cursor-pointer"
      ...
      >
        {(categories || []).map((cat) => {
          const catId = cat.id || cat._id;
          const isActive = activeCategory === catId || activeCategory === cat.name;
